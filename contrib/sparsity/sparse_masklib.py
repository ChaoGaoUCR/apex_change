...
def mn_1d_best(matrix, m, n, grad):
    ...
    # if mat.grad not None:
    
    pmax = torch.argmax(torch.matmul(mat.abs(),patterns.t()), dim=1) #compute threshold
    
    mask[:] = patterns[pmax[:]]
    mask = mask.view(matrix.shape)
    return mask

def m4n2_1d(mat, density,grad=False):
    # print("Test with dw*w and 7:8")
    # breakpoint()
    # return mn_1d_best(mat, 4, 2)
    return mn_1d_best(mat, 4, 3, grad)
...
def create_mask(tensor, pattern="m4n2_1d", density=0.5, tensor_grad = None):
    ...
    print("test how to add important score here")
    # breakpoint()
    if tensor_grad is not None:
        #important score change
        tensor = tensor * tensor_grad
    shape = tensor.shape
    ttype = tensor.type()
